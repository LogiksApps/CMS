				...
				<td width=310px>
					<select name=fieldtype style='width:90% !important;' onchange='checkHtmlButton(this)'>
						<optgroup label='Text Type'>
							...
							<option tag='input' tagtype='color' value='colorfield'>Color Field</option>

							<option tag='input' tagtype='url' value='urlfield'>URL Field</option>
							<option tag='input' tagtype='text' value='tagfield'>Tag Field</option>

							<option tag='input' tagtype='number' value='numberfield'>Number Field</option>
...
$(function() {
	$("#properties .pbtn").click(function() {
			...
			if($(this).hasClass("minibtnhtm")) {
				...
				forInput=$(this).prev().find("input[type=text]").get(0);
