...
if(isset($_REQUEST["action"])) {
	...
	if($_REQUEST["action"]=="viewtypelist") {
		echo "<option value=''>All</option>";
		$lfs=scandir($lf);
		unset($lfs[0]);unset($lfs[1]);
		foreach($lfs as $a) {
			if(is_dir($lf.$a)) echo "<option value='{$a}.'>$a</option>";
		}
		exit();
	} elseif($_REQUEST["action"]=="viewtable") {
		$xx=substr($_REQUEST['type'],0,strlen($_REQUEST['type'])-1);
		if(strlen($xx)>0) $base="{$xx}/";
		else $base="";
		scanTemplates($lf.$base,$xx);
		exit();
	} elseif($_REQUEST["action"]=="editurl" && isset($_REQUEST['tmpl'])) {
		$_REQUEST['tmpl']=str_replace(".","/",$_REQUEST['tmpl']);
		$p="{$lf}{$_REQUEST['tmpl']}.tpl";
		$p=str_replace($_SESSION["APP_FOLDER"]["APPROOT"],"",$p);
		echo $p;
		exit();
	} elseif($_REQUEST["action"]=="fetch" && isset($_REQUEST['tmpl'])) {
		$_REQUEST['tmpl']=str_replace(".","/",$_REQUEST['tmpl']);
		$tpl="{$lf}{$_REQUEST['tmpl']}.tpl";
		...
		if(strlen($_REQUEST['tmpl'])<=0) {
			exit("Template File Not Defined");
		}
		$_REQUEST['tmpl']=str_replace(".","/",$_REQUEST['tmpl']);
		$tpl="{$lf}{$_REQUEST['tmpl']}.tpl";
		$pth=substr($tpl,0,strlen($tpl)-4);
		...
		exit("Successfully Saved Template");
	} elseif($_REQUEST["action"]=="create" && isset($_REQUEST['tmpl'])) {
		$_REQUEST['tmpl']=str_replace(".","/",$_REQUEST['tmpl']);
		$_REQUEST['tmpl']=str_replace(" ","_",$_REQUEST['tmpl']);
		$tpl="{$lf}{$_REQUEST['tmpl']}.tpl";
		if(!is_dir(dirname($tpl))) {
				if(mkdir(dirname($tpl),0777,true))
					chmod(dirname($tpl),0777);
		}
		if(!is_dir(dirname($tpl))) {
			exit("Error Creating/Finding Template Base Type/Folder");
		}
		$pth=substr($tpl,0,strlen($tpl)-4);
		$sql=$pth.".sql";
		$css=$pth.".css";
...
function scanTemplates($lf,$dir="") {
	$lfs=scandir($lf);
	unset($lfs[0]);unset($lfs[1]);
	foreach($lfs as $a) {
		if(is_dir($lf.$a)) {
			scanTemplates("{$lf}{$a}/",$a);
		} else {
			$ext=strtolower(fileExtension($a));
			if($ext=="tpl") {
				if(strlen($dir)>0) $dir="{$dir}.";
				$t=substr($a,0,strlen($a)-4);//toTitle(substr($a,0,strlen($a)-4));
				$a=substr($a,0,strlen($a)-4);
				echo "<tr rel='{$dir}{$a}'><td align=center width=35px><input type=checkbox name=select rel='{$dir}{$a}' /></td><td>{$dir}{$a}</td></tr>";
			}
		}
	}
}
