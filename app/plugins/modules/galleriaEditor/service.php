	...
	if($_REQUEST["action"]=="listsets") {
		...
		foreach($fs as $p) {
			...
			if($fname==$_REQUEST['photo']) {
				...
				$lnk=$bannerFolder.$_REQUEST['photoset']."/lnks/{$fname}.lnk";
				break;
			}
		}
		if(strlen($f)>0) {
			...
			if(isset($_REQUEST['orginal']) && $_REQUEST['orginal']=="true") {
				...
				$html="<img src='$src' width=99% height=99% alt='No Media Found' title='$imgDimensions' />";
			}
			$imgData['html']=$html;
			...
			if(file_exists($lnk)) {
				$imgData["lnk"]=file_get_contents($lnk);
			} else {
				$imgData["lnk"]="";
			}
		...
		$f1=$bannerFolder.$_REQUEST['photoset']."/text/".$_REQUEST['photo'].".txt";
		$f2=$bannerFolder.$_REQUEST['photoset']."/lnks/".$_REQUEST['photo'].".lnk";
		
		if(!file_exists($f1) || is_writable($f1)) {
			file_put_contents($f1,$_REQUEST['data']);
		} else {
			echo "Sorry, Description Is ReadOnly.";
		}
		if(!file_exists($f2) || is_writable($f2)) {
			file_put_contents($f2,$_REQUEST['lnk']);
		} else {
			echo "Sorry, Link Is ReadOnly.";
		}
		exit();
		...
		$lnk=$bannerFolder.$_REQUEST['photoset']."/lnks/".$_REQUEST['photo'].".lnk";
		...
		if(file_exists($large) || file_exists($thumb) || file_exists($txt) || file_exists($lnk)) {
