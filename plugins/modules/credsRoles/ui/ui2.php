...
//printArray(RoleModel::getInstance()->getVersion());exit();

loadModule("pages");

function pageContentArea() {
    return file_get_contents(__DIR__."/templates/ui2.html");
}

$toolbar = [
            "selectRoles"=>["title"=>"Hidden Roles","type"=>"dropdown","align"=>"right","options"=>[]],
			"selectGUID"=>["title"=>"Select GUID","type"=>"dropdown","align"=>"right","options"=>[]],//"by_roles"=>"By Roles","by_plugins"=>"By Plugins"
    ...
    processAJAXPostQuery(_service("credsRoles","list-roles-permissions"), "roleids="+PRELOAD_ROLEIDS.join(","),function(data) {
            ...
            $.each(ROLE_PERMISSIONS, function(module,activities) {
                ...
                $.each(ROLE_LIST, function(a,role) {
                    ...
                    if(PRELOAD_ROLEIDS.length>0 && PRELOAD_ROLEIDS.indexOf(role.id)<0) return;
                ...
                $("#role-body").append(htmlBody);
                
                updateUIRowBlock($("#role-body").find(`tr.view[data-module='${module}'] td`));
            });
            ...
            $("#role-body .label-activity").click(function() {
...
function updateUIRowBlock(element) {
    ...
    if(privilegehash!=null) {
        $(`tr.role_row[data-module='${module}'].fold .role_ctrl_checkbox[data-privilegehash='${privilegehash}']`).each(function(k1, ele) {
    ...
    if(stxt==null || stxt.length<=0) {
        ...
        $(`tr.fold .positionSticky`).each(function() {
            if($(this).text().toLowerCase().trim().indexOf(stxt)>=0) {
                var module = $(this).closest("tr.role_row").data("module");
                $(`data-module='${module}'${module}]`).addClass("open");
...
function loadHiddenRoles() {
    ...
    HIDDEN_ROLES = localStorage.getItem("CREDROLES_HIDDEN_ROLES");
